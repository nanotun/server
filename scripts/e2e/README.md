...
- **不在这里做真还原。** 阶段 5 只验证 `restore` 的守卫会拒绝,不执行覆盖实时库的还原 ——
  ...
  如实重新敞开。
- **不测并发与规模。** 全程只有两个客户端会话,结构上摸不到并发问题。这块两头补:
  并发**正确性**由进程内 Go 压测钉死(下面),真机上**几十条会话同时在线时资源怎么走、
  撤压收不收得干净**由一条按需的三机 drill 走一遍(本条最后)。都不在门禁里:
  ...
  进程内压测证的是「锁内逻辑对不对」,摸不到真实传输、真 OS socket、真进程资源。那部分
  由一条按需的三机 drill 补:

  ```bash
  ./scripts/e2e/scale-drill.sh                 # 默认 40 条并发会话
  SCALE_N=60 SCALE_HOLD=120 ./scripts/e2e/scale-drill.sh
  ```

  它先停掉带 `--exit` 的 nanotun-a 腾出干净压力机,在 SRV 上建 N 个用户(客户端
  device_uuid 按机器派生,单机隔离 HOME 出不来第二台设备,只能换用户凑 N 条会话),
  从 A 起 N 个隔离 reality 客户端,靠**服务端自证的数字**断言:N 条会话
  distinct(v4)=distinct(v6)=distinct(dev)=N(租约不串台)、线程不随会话数线性增长
  (会话是 goroutine)、FD 满载 +N 撤压又回基线(每会话 socket 都释放 = 无泄漏),
  满载保持一段无 RSS 爬升。收尾自动删光压测用户、拉回 nanotun-a,中断也不留残。
  不进门禁:压力机得有客户端二进制,且会临时改动共用实验室的会话拓扑。
  (RSS 绝对值因登录期 argon2 峰值 + Go 惰性归还而发黏,不作每会话稳态解读;
  每会话稳态信号看 FD 与线程。)
- **不测安装链的漏译。** 安装链的文案是双语的(默认英文),而漏译只在**运行时**才看得见:
  `bash -n`、全量 `go test`、`--help` 的双语检查全绿,也照样可能有一句中文漏在装机第 6 步
  或收尾那段「常用运维 / 卸载」里 —— 2026-08-28 实测就是这样,唯一发现它的办法是真装一遍
  然后盯着屏幕。那件事由一条按需演练顶着:

  ```bash
  ./scripts/testlab/lab.sh i18n     # 全新容器里按英文装一遍 + 走向导,输出里出现中文即失败
  ```

  它把「盯着屏幕」变成一个退出码:装机 + 向导 + `nanotun-uninstall --dry-run` +
  `nanotun-set-suffix --help` 四段输出一起扫(后两段的语言来自装机时落盘的
  `/etc/nanotun/lang`,顺带验了那条路)。十几秒一轮。源码那侧的对应守卫在
  `cmd/nanotun-admin/scripts_i18n_guard_test.go`(默认必须英文、不许把默认按回 zh、
  `--help` 两种语言都得有、没有终端时不许问语言)。不进门禁:要 Docker。
...
- **MagicDNS 门禁只验服务端当前后缀;非默认后缀另有 drill。** 阶段 1(`10-exit.sh`)用的
  ...
  自动回滚),等 A 重连后从 A `dig @网关` 断言 `*.<新后缀>` 解析到 C 的 vIP、且**旧后缀
