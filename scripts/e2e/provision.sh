...
$out"
  ...
  # 运行期后缀**只**取服务端 config.toml 的 [server.magic_dns].domain_suffix(模板缺省 nanotun)——
  ...
  # 后缀由 e2e_resolve_magic_suffix() 开跑时从服务端读,阶段 1 开箱即绿。要验**非默认后缀**端到端,
  # 用 scripts/e2e/magic-suffix-drill.sh(改 config.toml→重启→验证客户端解析→还原,不污染门禁基线)。
  note "MagicDNS 后缀取 config.toml 的 domain_suffix(模板默认 nanotun);非默认后缀验证见 magic-suffix-drill.sh"
fi
