...
var (
	// LOCAL PATCH (vpn_port): upstream value is 8192. Some dests (e.g. www.microsoft.com
	// via Akamai) intermittently return a TLS Certificate record > 8192B, which tripped
	// "if handshakeLen > size { break f }" below and silently aborted the handshake
	// (client saw "TLS peer closed socket during handshake"). Raised to cover a full
	// 16KB TLS record plus headroom. Keep when bumping the xtls/reality dependency.
	size  = 32768
