...
//
// 这张证书是临场签发的(默认一百年,但 --hy2-client-cert-days 可以要短的,CA 剩余寿命也会
// 把它夹短),而二维码发出去之后没人再看它一眼。到期那天客户端的 Hy2/QUIC 通道握不上手,
// 退到其余传输 —— 用户感觉是「变慢了」而不是断线,排查时也很难联想到当初扫的那张码。
// 发的时候顺口说一句,比到时候翻日志便宜得多。
//
