...
func TestAttachEntryAddresses_IPv6Bracketed(t *testing.T) {
	cases := []struct {
		...
		// 两个端口都传 0 → 各自落默认。2026-08-28 起 reality 的默认也是 443,所以这一行
		// 期望的两个地址**故意**一样:REALITY 在 443/tcp、hy2 在 443/udp,协议不同不冲突,
		// 合起来正是支持 HTTP/3 的网站的指纹。看着像复制粘贴错了,其实不是。
		{"ipv6_default_ports", "2001:db8::1", 0, 0, "", "[2001:db8::1]:443", "[2001:db8::1]:443"},
