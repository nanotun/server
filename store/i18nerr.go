...
// 断言。因此本类型采用「双载」:
//
//   - Error() 返回英文(已格式化)—— CLI / 日志 / 测试看的都是这一份;
...
// i18nErr 构造可本地化错误:msg 给 Error()/CLI/日志/测试(英文),
// key+args 给 web 按语言翻译。
func i18nErr(key, msg string, args ...any) error {
	return &LocalizedError{msg: msg, key: key, args: args}
}
...
func i18nErrWrap(key, msg string, wrapped error, args ...any) error {
	return &LocalizedError{msg: msg, key: key, args: args, wrapped: wrapped}
}
