...
> **数据面进展**：数据面（server 把流量真正转发到宣告方 device、宣告方本机 NAT 进 LAN、请求方装路由）**已开始实现**，
> 里程碑与进度见 **`docs/PLAN_SUBNET_ROUTE_DATAPLANE.md`**（SR-M1 服务器数据面已落地并提交；SR-M2 宣告方客户端代码
> 完成）。下文 §3「数据面(本期未实现)」/ §5「客户端实现要点」是当初的接入设想，实际实现以 PLAN 文件为准。
