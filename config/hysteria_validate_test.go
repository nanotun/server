...
func TestValidateHysteriaCredentials_AllOrNothingAndNoWeakPassword(t *testing.T) {
	...
	cases := []struct {
		...
		{"只配密码", HysteriaConfig{Password: okPW}, "must all be set",
			"半配起不来监听,而运维会以为 hy2 已经在跑"},
		{"只配证书", HysteriaConfig{TLSCertFile: "/c.pem"}, "must all be set", ""},
		{"配了密码和证书,漏了私钥", HysteriaConfig{Password: okPW, TLSCertFile: "/c.pem"}, "must all be set", ""},
		{"空白不算配置", HysteriaConfig{Password: "   ", TLSCertFile: "/c.pem", TLSKeyFile: "/k.pem"}, "must all be set",
			"只有空白的密码等于没配,不能让它凑数把三项凑齐"},

		{"密码 15 字节", HysteriaConfig{Password: strings.Repeat("a", 15), TLSCertFile: "/c.pem", TLSKeyFile: "/k.pem"},
			"at least 16 bytes", "hy2 只有这一道门,短口令等于敞开"},
		...
		{"obfs 密码但没配 hy2", HysteriaConfig{ObfsSalamanderPassword: "abcd"}, "to be set first",
			"混淆是 hy2 之上的一层,单独开没有意义"},
		{"obfs 密码太短", HysteriaConfig{Password: okPW, TLSCertFile: "/c.pem", TLSKeyFile: "/k.pem",
			ObfsSalamanderPassword: "abc"}, "at least 4 bytes", "库本身的下限,配短了是启动期报错"},
