...
3. **Install** ([`install-self-hosted.sh`](scripts/install-self-hosted.sh)) — systemd units,
   IP forwarding, REALITY / hy2 keys and self-signed certs, opens ufw, the first client admin
...
The client account and the Web admin are **two separate things**, the easiest to confuse: the
...
The image is `ghcr.io/nanotun/server` (amd64 + arm64 multi-arch). It's a mesh gateway with hard
...
The data plane listens on `[server].listen_addr` (default `127.0.0.1:8080`, loopback only),
...
machine — it defaults the dial address to the current value, and skips the web admin and the client
